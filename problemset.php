...
while ($row=mysql_fetch_object($result)){
	...
	if (isset($sub_arr[$row->problem_id])){
		if (isset($acc_arr[$row->problem_id])) 
			$view_problemset[$i][0]="<span class='label label-success'>성공</span>";
		else 
			$view_problemset[$i][0]="<span class='label label-danger'>실패</span>";
	}else{
		$view_problemset[$i][0]= "";
	}
	$view_problemset[$i][1] = $row->problem_id;
	$view_problemset[$i][2] ="<a href='problem.php?id=".$row->problem_id."'>".$row->title."</a>";
	$view_problemset[$i][3] ="<nobr>".mb_substr($row->source,0,8,'utf8')."</nobr>";
	$view_problemset[$i][4] ="<a href='status.php?problem_id=".$row->problem_id."&jresult=4'>".$row->accepted."</a>";
	$view_problemset[$i][5] ="<a href='status.php?problem_id=".$row->problem_id."'>".$row->submit."</a>";
